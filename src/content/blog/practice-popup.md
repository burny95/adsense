...
브라우저가 배경 영역의 클릭을 `<dialog>` 요소로 넘겨주기 때문에, **배경을
클릭하면 `e.target`이 `dialog`가 됩니다.** 팝업 상자가 화면 전체를 덮고 있어서가
아닙니다 — 상자는 내용만큼만 차지하고, 그 바깥을 눌러도 대상은 `dialog`입니다.
안쪽 내용을 클릭하면 그 요소가 target이 되므로 구분이 됩니다.
