...
신입 때 이 문제로 QA 티켓을 두 번 받았습니다. 처음 올라온 제목은 "주문 팝업이
가끔 저절로 닫힘"이었습니다. 재현이 안 돼서 한참 붙들고 있었는데, 이유는
싱거웠습니다. 제가 확인할 때는 늘 제목이나 버튼을 누르고 있었습니다. QA가
붙여 준 화면 녹화를 보고서야 알았습니다. 그쪽은 팝업 모서리 근처, 아무것도
없는 여백을 누르고 있었습니다.

범인은 브라우저 기본 스타일의 `padding`이었고 고치는 데는 한 줄이면 됐지만,
그 뒤로 습관이 하나 생겼습니다. **바깥 클릭으로 닫기를 붙였으면 내용이 아니라
가장자리를 눌러 본다.** 네 귀퉁이를 한 번씩 눌러 보는 데 5초면 됩니다.
...
이건 실기기를 만지기 전까지 몰랐습니다. 문의하기 팝업에 이름·연락처·내용 세
칸을 넣었는데 개발자도구의 모바일 모드에서는 아무 문제가 없었습니다. 그런데
화면이 작은 기기에서 열어 보니, 내용 칸을 누르는 순간 키보드가 올라오면서 그
칸이 키보드 뒤로 완전히 숨었습니다. 무엇을 치고 있는지 보이지 않는 채로
타이핑을 하게 되는 상태였습니다.

원인은 확인 방법에 있었습니다. **개발자도구의 모바일 모드는 화면 크기만 흉내
낼 뿐 키보드를 올려 주지 않습니다.** 폭만 좁혀 놓고 "모바일 확인 끝"으로
넘겼던 것입니다. 그 뒤로 입력 칸이 들어간 팝업만큼은 실기기에서, 그것도
화면이 작은 쪽으로 한 번은 열어 봅니다.
...
저도 그렇게 한 번 크게 데었습니다. 데스크톱에서 몇 번을 돌려도 멀쩡하던 주문
팝업이, 검수 날 폰에서 열자마자 두 가지가 같이 터졌습니다. 팝업을 열어 둔 채
화면을 쓸어 올리니 뒤 페이지가 따라 움직였고, 뒤로가기를 눌렀더니 팝업이
닫히는 게 아니라 이전 페이지로 넘어가 버렸습니다. 둘 다 애초에 코드에 없던
기능이었는데, 데스크톱에서는 없다는 사실이 드러날 일이 없었던 것뿐입니다.

그래서 지금은 팝업 작업이 끝나면 **폰을 꺼내는 것부터** 합니다. 뒤 배경을 한
번 쓸어 보고, 뒤로가기를 한 번 눌러 봅니다. 위 목록의 여덟 줄 중에 이 둘만은
코드를 아무리 들여다봐도 보이지 않습니다.
